 ...
 *   TEST_HARNESS_MAIN
 *
 * When running tests from a kernel module, each test must be
 * explicitly registered at runtime.
 *   int module_init(void)
 *   {
 *      REGISTER_TEST(standalone_test);
 *      REGISTER_FIXTURE(my_fixture);
 *      REGISTER_TEST_F(my_fixture, data_is_good);
 *      @@@@@
 *   }
 *
...
#define REGISTER_TEST TEST_API(REGISTER_TEST)
#define REGISTER_FIXTURE TEST_API(REGISTER_FIXTURE)
#define REGISTER_TEST_F TEST_API(REGISTER_TEST_F)
...
#  define _REGISTER_TEST(test_name) __register_test(&_##test_name##_object)
/* Can only use _initcall for registration in non-module code; */
/* module code must explicitly register tests */
#ifndef MODULE
#  define __TEST_REGISTER(test_name) \
    static int _register_##test_name(void) { \
      _REGISTER_TEST(test_name); \
      return 0; \
    } \
...
#ifdef __KERNEL__
#  define _REGISTER_FIXTURE(fixture_name) __fixture_count++
/* Can only use _initcall for registration in non-module code; */
/* module code must explicitly register fixtures */
#ifndef MODULE
#  define __FIXTURE_REGISTER(fixture_name) \
    static int _register_##fixture_name##_data(void) { \
      _REGISTER_FIXTURE(fixture_name); \
      return 0; \
    } \
...
#  define _FIXTURE(fixture_name) \
    __FIXTURE_REGISTER(fixture_name) \
...
#ifdef __KERNEL__
#define _REGISTER_TEST_F(fixture_name, test_name) __register_test(&_##fixture_name##_##test_name##_object)
/* Can only use _initcall for registration in non-module code; */
/* module code must explicitly register tests */
#ifndef MODULE
#  define __TEST_F_REGISTER(fixture_name, test_name) \
    static int _register_##fixture_name##_##test_name(void) { \
      _REGISTER_TEST_F(fixture_name, test_name); \
      return 0; \
    } \
...
#  define __TEST_F_IMPL(fixture_name, test_name, signal) \
    ...
    __TEST_F_REGISTER(fixture_name, test_name) \
    static void fixture_name##_##test_name( \
      struct __test_metadata __attribute__((unused)) *_metadata, \
      _FIXTURE_DATA(fixture_name) __attribute__((unused)) *self, \
